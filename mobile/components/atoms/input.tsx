...
export default function Input<T>({onBlur, onChange, onFocus, placeholder, value, type = 'text', returnKeyType, small = false}: InputProps<T>) {
    ...
    if(type === 'date') {
        const dateValue = value ? value : new Date();
        
        function handleChange(event: any, value: Date) {
            onChange?.(value);
        }

        return <View style={tw(`bg-white rounded-full ${!small ? 'px-8 h-14 w-full' : 'px-2 h-8 flex-1'} my-4 flex flex-row items-center`)}>
            <Text>{placeholder}</Text>
            <RNDateTimePicker accentColor="#5b21b6" mode="date" minimumDate={new Date(1900, 0, 1)} value={dateValue} onChange={handleChange}/>
        </View>
    }

    return <View style={tw(`bg-white rounded-full ${!small ? 'px-8 h-14 w-full' : 'px-2 h-10 flex-1'} my-4 flex flex-row`)}>
        <TextInput style={tw(`flex-1 ${!small ? 'h-14' : 'h-10'}`)} placeholderTextColor='gray' onChangeText={onChange} onBlur={onBlur} placeholder={placeholder} value={value} onFocus={onFocus} secureTextEntry={password && !visible} keyboardType={getKeyboardType(type)} passwordRules={passwordRules} returnKeyType={returnKeyType} autoCapitalize={autoCaps ? "sentences" : "none"}/>
